...
// Where one person's files live. `uid` is absent only in the server harness's
// oldest fixtures, which predate there being more than one person.
const homes = (uid) => ({ media: uid ? `media/${uid}` : "media", originals: uid ? `originals/${uid}` : "originals" });

// The three photo tiers from any raster buffer (a photo, or a video's poster frame).
// `home` is the owner's media folder: two people who upload the same bytes must
// not share one derivative, or the first to delete breaks the other's gallery.
async function tiers(buf, dataDir, hash, home) {
  ...
  await mkdir(path.join(dataDir, home), { recursive: true });
  const rel = { large: `${home}/${hash}-${LARGE}.webp`, medium: `${home}/${hash}-${MEDIUM}.webp`, thumb: `${home}/${hash}-${THUMB}.webp` };
...
export async function ingestVideo(buf, filename, { dataDir, email, uid }) {
  const home = homes(uid);
  const hash = createHash("sha256").update(buf).digest("hex").slice(0, 16);
  const id = `p${hash}`;
  const ext = (path.extname(filename || "").toLowerCase().replace(/[^a-z0-9.]/g, "") || ".mp4").slice(0, 8);
  const originalRel = `${home.originals}/${hash}${ext}`;
  const originalAbs = path.join(dataDir, originalRel);
  if (await exists(originalAbs)) return { id, duplicate: true };
  await mkdir(path.join(dataDir, home.originals), { recursive: true });
  await mkdir(path.join(dataDir, home.media), { recursive: true });
  ...
  try {
    ...
    const poster = await tiers(await posterFrame(originalAbs, info.duration), dataDir, hash, home.media);
    const srcRel = `${home.media}/${hash}-${VIDEO_MAX_EDGE}.mp4`;
...
export async function ingestPhoto(buf, filename, { dataDir, email, uid }) {
  const home = homes(uid);
  const hash = createHash("sha256").update(buf).digest("hex").slice(0, 16);
  const id = `p${hash}`;
  const ext = (path.extname(filename || "").toLowerCase().replace(/[^a-z0-9.]/g, "") || ".jpg").slice(0, 8);
  const originalRel = `${home.originals}/${hash}${ext}`;
  const largeRel = `${home.media}/${hash}-${LARGE}.webp`;
  const mediumRel = `${home.media}/${hash}-${MEDIUM}.webp`;
  const thumbRel = `${home.media}/${hash}-${THUMB}.webp`;
...
export async function backfillMedium(lib, dataDir) {
  const todo = (await lib.moments()).filter((m) => m.media?.type === "photo" && !m.media.medium && m.media.src);
  const made = new Map();
  for (const m of todo) {
  ...
  if (made.size) await lib.updateMoments((ms) => ms.map((x) => (made.has(x.id) && !x.media.medium ? { ...x, media: { ...x.media, medium: made.get(x.id) } } : x)));
  return made.size;
}
