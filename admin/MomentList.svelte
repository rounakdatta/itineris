<script>
  ...
  let { moments, selectedId = null, onSelect } = $props();

  // Newest day first -- what you just uploaded is what you want to tag.
  const groups = $derived(
    Object.entries(
      moments.reduce((acc, m) => { (acc[dayKey(m.t)] ??= []).push(m); return acc; }, {})
    ).sort(([a], [b]) => (a < b ? 1 : -1))
  );
</script>
...
{#each groups as [day, items] (day)}
  ...
  <div class="grid">
    {#each items as m (m.id)}
      <button class="cell" class:on={m.id === selectedId} onclick={() => onSelect(m.id)} title={m.caption || m.filename || m.id}>
...
{/each}
{#if moments.length === 0}
  <p class="muted">No moments yet. Add some photos above.</p>
{/if}
...
<style>
  ...
  .cell {
    position: relative; aspect-ratio: 3 / 4; padding: 0; border: 2px solid transparent; border-radius: 10px;
    overflow: hidden; background: var(--panel); text-align: left;
  }
  .cell.on { border-color: var(--accent); }
  .cell img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .t { position: absolute; left: 7px; bottom: 6px; font-size: 11px; color: #fff; text-shadow: 0 1px 4px rgba(0,0,0,.9); font-variant-numeric: tabular-nums; }
  ...
  .tags {
    position: absolute; left: 0; right: 0; top: 0; padding: 26px 7px 0; font-size: 10px; color: #fff;
    background: linear-gradient(to bottom, rgba(0,0,0,.6), transparent); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
</style>
