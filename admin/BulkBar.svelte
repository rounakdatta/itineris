<script>
  ...
  let { selection, galleries, suggestions = [], known = [], placesEnabled = false, onDone, onExit } = $props();
...
<div class="bulk" role="region" aria-label="Bulk actions">
  <div class="row">
    ...
    {#if !mode}
      ...
      <button class="btn small" onclick={() => { mode = "location"; lat = ""; lng = ""; place = ""; mapsUrl = null; placeId = null; }}>Location</button>
    ...
    {:else if mode === "location"}
      <span class="muted small">Pin all {ids.length} to one place: one of your places, a Google Maps search or link, your location, or a spot on the map.</span>
      <div class="loc">
        <MapPicker lat={lat === "" ? null : +lat} lng={lng === "" ? null : +lng} {known} {placesEnabled} onChange={(a, b) => { lat = a; lng = b; }} onPlace={(name) => (place = name)} onLink={(u) => (mapsUrl = u)} onPick={(p) => (placeId = p?.placeId ?? null)} />
