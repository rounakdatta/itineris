...
describe("Outbox: a place for the whole batch", () => {
  it("photos added at a shared place carry it -- over EXIF -- and it travels to the server", async () => {
    ...
    await ob.add([file("a.jpg")], { location: { lat: 24.471235, lng: 54.371235, name: "Zahrat Lebnan", mapsUrl: "https://maps.google.com/?cid=5", placeId: "ChIJzahrat" } });
    const it = ob.items[0];
    expect(it.meta).toMatchObject({ lat: 24.471235, lng: 54.371235, place: "Zahrat Lebnan", mapsUrl: "https://maps.google.com/?cid=5", placeId: "ChIJzahrat", locEdited: true });
    expect(metaToSend(it)).toEqual({ place: "Zahrat Lebnan", lat: 24.471235, lng: 54.371235, mapsUrl: "https://maps.google.com/?cid=5", placeId: "ChIJzahrat" });
